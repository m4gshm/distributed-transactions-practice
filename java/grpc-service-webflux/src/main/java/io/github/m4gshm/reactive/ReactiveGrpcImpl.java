...
public class ReactiveGrpcImpl implements ReactiveGrpc {
    ...
    public <P extends CorePublisher<T>, T> void subscribe(
        ...
        var observation = traceService.startNewObservation(spanName);
        try (var _ = observation.openScope()) {
            var publisher = publisherFactory.get();
            final P p;
            if (publisher instanceof Mono<?> mono) {
                p = (P) mono.name(name)
                        .contextWrite(context -> traceService.putToReactContext(context, observation));
