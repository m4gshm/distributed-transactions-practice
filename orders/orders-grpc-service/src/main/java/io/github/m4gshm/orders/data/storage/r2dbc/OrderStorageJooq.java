...
import static io.github.m4gshm.orders.data.storage.r2dbc.OrderStorageJooqQueryUtils.storeOrder;
...
public class OrderStorageJooq implements OrderStorage {
    ...
    public Mono<Order> save(Order order, boolean twoPhasedCommit) {
        return from(dsl.transactionPublisher(trx -> {
            ...
            return !twoPhasedCommit ? routine : TwoPhaseTransaction.prepare(dsl, order.id(), routine);
        }));
    }
}
