...
import (
	...
	tpcpb "github.com/m4gshm/distributed-transactions-practice/golang/gen/go/tcp"
...
func (s *Service) ListActives(ctx context.Context, req *tpcpb.TwoPhaseListActivesRequest) (*tpcpb.TwoPhaseListActivesResponse, error) {
	...
	if err != nil {
		return nil, status.Errorf(codes.Internal, "failed to list active transactions: %v", err)
	}
	defer rows.Close()
...
func (s *Service) Commit(ctx context.Context, req *tpcpb.TwoPhaseCommitRequest) (*tpcpb.TwoPhaseCommitResponse, error) {
	...
	if err != nil {
		return nil, status.Errorf(codes.Internal, "failed to check transaction: %v", err)
	}
